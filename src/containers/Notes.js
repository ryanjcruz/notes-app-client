...
class Notes extends Component {
  ...
  // save the note by making a PUT request
  // to the specific note with its note id
  saveNote(note) {
    return invokeApig({
  ...
  handleSubmit = async (event) => {
    ...
    // if there's a file to upload call s3Upload
      ...
      <div className="Notes">
        { this.state.note &&
          ( <form onSubmit={this.handleSubmit}>
              ...
              {/* this is conditionally rendered if there's an attachment */}
              { this.state.note.attachment &&
              ( <FormGroup>
                  <ControlLabel>Attachment</ControlLabel>
                  <FormControl.Static>
                    <a target="_blank" href={ this.state.note.attachment }>
                      {/* format attachment using formatFilename since S3 gives long URLs */}
                ...
                <LoaderButton 
                  ...
                  text="Save"
                  loadingText="Saving..." />
                  {/* Delete button added to delete notes
                  confirm using the browser's delete dialog */}
                <LoaderButton 
                  block
                  bsStyle="danger"
                  bsSize="large"
